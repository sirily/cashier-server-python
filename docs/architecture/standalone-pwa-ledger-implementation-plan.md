...
4. Только после зелёного Docker image и проверки RustLedger — PR на ревью.
5. Merge выполняется только после явного разрешения владельца репозитория; зелёный CI сам по себе не является разрешением на merge.
...
Fixture обязан воспроизводить форму production root, включая prose/org-style заголовок первой строкой файла до первой Beancount option:
...
Этот случай должен проверяться отдельным regression test через endpoint `/infrastructure?file_path=main.bean`: до исправления он воспроизводит `422 Unsupported non-entry source directive at main.bean:1`, после исправления возвращает автономный snapshot, который проходит `@rustledger/wasm@0.14.1`.

Существующий `tests/fixtures/plugin_ledger` также должен получить операции с реальной проблемной семантикой:
